...
C ---    Add labels for non-DFAULT ALPHA and BETA Options
         ...
         ELSE IF (.NOT. DFAULT .AND. BETA) THEN
            MODOPS(19) = 'BETA'
         ELSE IF (.NOT. DFAULT .AND. BETA) THEN
            CALL ERRHDL(PATH,MODNAM,'E','137','BETA')
         ELSE IF (DFAULT .AND. L_ALPHA) THEN
            CALL ERRHDL(PATH,MODNAM,'E','138','ALPHA')
         END IF
...
C ---          Assign LOW_WIND = .TRUE. 
            ...
            ELSE
               LOW_WIND = .FALSE.
               CALL ERRHDL(PATH,MODNAM,'E','LWD','NonDFAULT')
            END IF
...
C           for FRANMAX on the LOW_WIND keyword
            IF (.NOT. L_UserFRANmax) THEN
               CALL ERRHDL(PATH,MODNAM,'W','133','0.9D0    ') 
            END IF
...
C --- Open MODEL, METEOR, AREA, and PRIME debug files, if selected; 
C     note that PVMRM, OLM, ARM2, and DEPOS debug files are opened
C     elsewhere
